...
static int word_mod_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    uint32_t* p = data;
    if (reason == MENU_CALLBACK_THINK_INACTIVE) {
        if (menu_intinput_get(item) != *p) {
            menu_intinput_set(item, *p);
...
static _Bool heap_node_validate(z64_arena_node_t* node) {
    ...
    return node->magic == 0x7373 &&
           (!next || (next >= 0x80000000 && next < 0x80400000 && next > addr && next - addr == size)) &&
           (!prev || (prev >= 0x80000000 && prev < 0x80400000 && prev < addr));
}

static int heap_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    int x = draw_params->x;
    int y = draw_params->y;
    ...
    gfx_printf(font, x, y + ch * 0, "node count     %i (%i free)", node_count_total, node_count_free);
    ...
    if (error_node) {
        gfx_printf(font, x, y + ch * 9, "erroneous node 0x%08" PRIx32, (uint32_t)error_node);
    }
    /* plot graph */
    ...
    while (p) {
        ...
        for (size_t i = b; i < e; ++i) {
            ...
            if (p->free) {
                ...
                c = GPACK_RGBA5551(0x1F, 0x1F - p->size * 0x1F / max_alloc, i == b ? 0x1F : 0x00, 0x01);
            }
            d[i] = c;
        }
        p = p->next;
    ...
    for (int i = 0; i < t->tiles_x; ++i) {
        struct gfx_sprite s = {
            t, i, x + t->tile_width * i, y + ch * 10, 1.f, 16.f,
        };
        gfx_sprite_draw(&s);
    }
    return 1;
...
static int disp_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    int x = draw_params->x;
    int y = draw_params->y;
    struct gfx_font* font = draw_params->font;
    uint32_t color = draw_params->color;
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        gfx_printf(font, x + cw * 2, y + ch * (i * 2 + 1), "0x%04" PRIx32 "  0x%04" PRIx32, gz.disp_hook_size[i],
                   gz.disp_hook_p[i]);
        gfx_printf(font, x + cw * 2 + 192, y + ch * (i * 2 + 1), "0x%04" PRIx32,
                   gz.disp_hook_size[i] - gz.disp_hook_d[i]);
        /* draw graph */
        gfx_mode_set(GFX_MODE_COLOR, GPACK_RGBA8888(0xFF, 0xFF, 0xFF, alpha));
        for (int j = 0; j < t[i]->tiles_x; ++j) {
            struct gfx_sprite s = {
                t[i], j, x + cw * 2 + 120 + t[i]->tile_width * j, y + ch * i * 2 + 1, 1.f, ch - 2,
            };
            gfx_sprite_draw(&s);
        }
    }
...
static void push_object_proc(struct menu_item* item, void* data) {
    ...
    size_t size = z64_object_table[object_id].vrom_end - z64_object_table[object_id].vrom_start;
...
static int objects_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    int x = draw_params->x;
    int y = draw_params->y;
    struct gfx_font* font = draw_params->font;
    uint32_t color = draw_params->color;
    ...
    size_t max_objects = sizeof(z64_game.obj_ctxt.objects) / sizeof(*z64_game.obj_ctxt.objects) - 1;
    gfx_printf(font, x, y + ch * 0, "objects   %i / %i", z64_game.obj_ctxt.n_objects, max_objects);
    ...
    for (int i = 0; i < z64_game.obj_ctxt.n_objects; ++i) {
        gfx_printf(font, x + cw * (i % 2) * 16, y + ch * (i / 2 + 5), "%08" PRIx32 " %04" PRIx16,
                   z64_game.obj_ctxt.objects[i].data, z64_game.obj_ctxt.objects[i].id);
    }
    return 1;
}
...
static int actor_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    ...
    if (adi->index < max) {
...

        {
            Mtx m;
            {
                ...
                MtxF mt;
                guRotateRPYF(&mt, actor->rot_2.x * M_PI / 0x8000, actor->rot_2.y * M_PI / 0x8000,
            ...
            gSPMatrix(z64_ctxt.gfx->poly_xlu.p++, gDisplayListData(&z64_ctxt.gfx->poly_xlu.d, m),
...
static void spawn_actor_proc(struct menu_item* item, void* data) {
    ...
    z64_SpawnActor(&z64_game.actor_ctxt, &z64_game, asi->actor_no, asi->x, asi->y, asi->z, asi->rx, asi->ry, asi->rz,
                   asi->variable);
}
...
static void spawn_actor_attached_b_proc(struct menu_item* item, void* data) {
    ...
    if (ai->adi->index < z64_game.actor_list[ai->adi->type].length) {
        ...
        z64_SpawnActorAttachedB(&z64_game.actor_ctxt, actor, &z64_game, ai->asi->actor_no, ai->asi->x, ai->asi->y,
                                ai->asi->z, ai->asi->rx, ai->asi->ry, ai->asi->rz, ai->asi->variable);
    }
}
...
#ifndef WIIVC
static void start_rdb_proc(struct menu_item* item, void* data) { rdb_start(); }

static void stop_rdb_proc(struct menu_item* item, void* data) { rdb_stop(); }

static void break_proc(struct menu_item* item, void* data) { rdb_interrupt(); }

static int rdb_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    ...
    gfx_printf(draw_params->font, draw_params->x, draw_params->y, "rdb status: %s", status);
    return 1;
}
#endif
...
struct menu* gz_debug_menu(void) {
    ...
    item = menu_add_static_custom(&actors, 0, 2, actor_draw_proc, NULL, 0xC0C0C0);
    item->data = &adi;
    ...
    menu_add_button(&actors, 0, 12, "fetch from link", &fetch_actor_info_proc, &asi);
