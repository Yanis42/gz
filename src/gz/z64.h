...
/* functions */
void z64_Actor_UpdateBgCheckInfo(z64_game_t* game, z64_actor_t* actor, float wall_check_height, float wall_check_radius,
                                 float ceiling_check_height, int32_t flags);
void z64_DrawActors(z64_game_t* game, void* actor_ctxt);
void z64_DeleteActor(z64_game_t* game, void* actor_ctxt, z64_actor_t* actor);
void z64_SpawnActor(void* actor_ctxt, z64_game_t* game, uint16_t actor_id, float x, float y, float z, uint16_t rx,
                    uint16_t ry, uint16_t rz, uint16_t variable);
z64_actor_t* z64_SpawnActorAttachedB(void* actor_ctxt, z64_actor_t* actor, z64_game_t* game, uint16_t actor_id, float x,
                                     float y, float z, uint16_t rx, uint16_t ry, uint16_t rz, uint16_t variable);
void z64_CreateStaticCollision(z64_col_ctxt_t* col_ctxt, z64_game_t* game, z64_col_lut_t* col_lut);
...
void z64_LoadActionLabel(z64_if_ctxt_t* if_ctxt, uint16_t action_idx, int button_idx);
void z64_UpdateEquipment(z64_game_t* game, z64_link_t* link);
void z64_InitPauseObjects(z64_game_t* game, void* addr, void* s72C);
void z64_LoadRoom(z64_game_t* game, z64_room_ctxt_t* room_ctxt, uint8_t room_index);
void z64_DrawRoom(z64_game_t* game, z64_room_t* room, int unk_a2);
void z64_UnloadRoom(z64_game_t* game, z64_room_ctxt_t* room_ctxt);
void z64_Sram_LoadDebugSave(void);
void z64_Io(uint32_t dev_addr, void* dram_addr, uint32_t size, int32_t direction);
void z64_CreateSkyGfx(z64_sky_ctxt_t* sky_ctxt, int skybox_type);
void z64_CreateSkyVtx(z64_sky_ctxt_t* sky_ctxt, int a1);
void z64_StopSfx(void);
void z64_UpdateCtxtInput(z64_ctxt_t* ctxt);
void z64_GetInput(void* input_ctxt, z64_input_t* input, int a2);
...
void z64_Audio_PlaySfxGeneral(uint16_t sfx_id, z64_xyzf_t* pos, uint8_t token, float* freq_scale, float* vol,
                              int8_t* reverb_add);
uint8_t z64_Audio_IsSfxPlaying(uint32_t sfx_id);
int z64_CheckAfxConfigBusy(void);
uint32_t z64_LoadOverlay(uint32_t vrom_start, uint32_t vrom_end, uint32_t vram_start, uint32_t vram_end, void* dst);
