...
static OSThread* const rdb_threads[] = {
    ...
    &z64_thread_fault,
#endif
    &z64_thread_main,   &z64_thread_graph,
#if defined(RDB_DEBUG_SCHED) && RDB_DEBUG_SCHED
    &z64_thread_sched,
#endif
    &z64_thread_padmgr, &z64_thread_audio, &z64_thread_dmamgr, &z64_thread_irqmgr,
};
...
static const char* const rdb_thread_name[] = {
    ...
    "sched",
#endif
    "padmgr", "audio", "dmamgr", "irqmgr",
};

static struct {
    _Alignas(0x10) struct fifo_pkt ipkt;
    _Alignas(0x10) struct fifo_pkt opkt;
    int ipos;
...
static _Bool rdb_poll(void) { return fifo_poll(); }
...
static void rdb_disable_watch(void) { __asm__("mtc0    $zero, $18;"); }

static int rdb_nthreads(void) { return sizeof(rdb_threads) / sizeof(*rdb_threads); }
...
__attribute__((optimize(0))) static OSThread* rdb_continue(void) {
    rdb_enable_watch();
    rdb_startall();
...
static void rdb_stop_reply(OSThread* thread) {
    ...
    switch ((thread->context.cause >> 2) & 0x1F) {
        ...
        case 7: /* dbe */
            ...
            break;
        case 13: /* tr */
            if (check_addr(thread->context.pc, 4) && *(uint32_t*)thread->context.pc == RDB_INTR_OP) {
                thread->context.pc += 4;
                if (rdb.detach) {
                    rdb_putpkt(0, "X0F");
                    return;
        ...
        case 23: /* watch */
            if (rdb.watch_active) {
                switch (rdb.watch_type) {
                    case 1:
                        sprintf(rs, "watch:%08" PRIx32 ";", rdb.watch_addr);
                        break;
                    case 2:
                        sprintf(rs, "rwatch:%08" PRIx32 ";", rdb.watch_addr);
                        break;
                    case 3:
                        sprintf(rs, "awatch:%08" PRIx32 ";", rdb.watch_addr);
                        break;
                }
            }
            sig = SIGTRAP;
    ...
    rdb_putpkt(0, "T%02" PRIx32 "%s25:%016" PRIx64 ";%s", sig, ts, rdb_get_reg(thread, 0x25), rs);
}
...
static void rdb_main(void* arg) {
    ...
    if (fault_event_set) {
        ...
        osCreateMesgQueue(&rdb_fault_mq, rdb_fault_mesg, sizeof(rdb_fault_mesg) / sizeof(*rdb_fault_mesg));
        osSetEventMesg(OS_EVENT_CPU_BREAK, &rdb_fault_mq, (OSMesg)OS_EVENT_CPU_BREAK);
        osSetEventMesg(OS_EVENT_FAULT, &rdb_fault_mq, (OSMesg)OS_EVENT_FAULT);
        fault_event_set = 1;
    }
    ...
    while (!rdb.detach) {
        ...
        if (strncmp(pkt, "qSupported", 10) == 0) {
            ...
            rdb_putpkt(0, "l");
        } else if (sscanf(pkt, "qThreadExtraInfo,%" SCNx32 "%n", &addr, &n) == 1 && pkt[n] == 0) {
            int thread_idx = -1;
            for (int i = 0; i < rdb_nthreads(); ++i) {
        ...
        } else if (sscanf(pkt, "H%[cg]%" SCNx32 "%n", &cmd, &addr, &n) == 2 && pkt[n] == 0) {
            if (addr == 0) {
                rdb_putpkt(0, "OK");
            } else {
        ...
        } else if (sscanf(pkt, "T%" SCNx32 "%n", &addr, &n) == 1 && pkt[n] == 0) {
            OSThread* thread = rdb_thread_by_id(addr);
            if (thread) {
                rdb_putpkt(0, "OK");
            } else {
        ...
        } else if (sscanf(pkt, "P%" SCNx32 "=%16" SCNx64 "%n", &addr, &value, &n) == 2 && pkt[n] == 0) {
            rdb_set_reg(rdb.gthread, addr, value);
            rdb_putpkt(0, "OK");
        } else if (sscanf(pkt, "m%" SCNx32 ",%" SCNx32 "%n", &addr, &length, &n) == 2 && pkt[n] == 0) {
        ...
        } else if (sscanf(pkt, "M%" SCNx32 ",%" SCNx32 "%n", &addr, &length, &n) == 2 && pkt[n] == ':') {
            if (check_addr(addr, 1)) {
                char* p = &pkt[n + 1];
                uint32_t i;
                for (i = 0; i < length; ++i) {
                    uint32_t value;
                    if (check_addr(addr, 1) && sscanf(p, "%2" SCNx32 "%n", &value, &n) == 1 && n == 2) {
                        *(uint8_t*)addr++ = value;
                        p += n;
                    } else {
                        break;
        ...
        } else if (sscanf(pkt, "%1[Zz]%1[0234],%" SCNx32 ",%" SCNx32 "%n", &cmd, &type, &addr, &length, &n) == 4 &&
                   pkt[n] == 0) {
            if (type == '0') {
                if (length != 4) {
                    rdb_putpkt(0, "E00");
                ...
                if (cmd == 'Z') {
                    if (rdb.watch_active) {
                        if (rdb.watch_addr == addr && rdb.watch_length == length && rdb.watch_type == t) {
                            rdb_putpkt(0, "OK");
                        } else {
                            rdb_putpkt(0, "E00");
                        }
                    ...
                    if (rdb.watch_active && rdb.watch_addr == addr && rdb.watch_length == length &&
                        rdb.watch_type == t) {
                        rdb.watch_active = 0;
                    }
                    rdb_putpkt(0, "OK");
        ...
        } else if (strcmp(pkt, "c") == 0 || (sscanf(pkt, "C%" SCNx32 "%n", &addr, &n) == 1 && pkt[n] == 0)) {
            rdb.cthread = rdb_continue();
            rdb.gthread = rdb.cthread;
            rdb_stop_reply(rdb.cthread);
        } else if (strcmp(pkt, "s") == 0 || (sscanf(pkt, "S%" SCNx32 "%n", &addr, &n) == 1 && pkt[n] == 0)) {
...
void rdb_start(void) {
    ...
    static __attribute__((section(".stack"))) _Alignas(8) char rdb_stack[0x1000];

    int irqf = set_irqf(0);
    if (!rdb_active) {
        rdb_active = 1;
        osCreateThread(&rdb_thread, 0, rdb_main, NULL, &rdb_stack[sizeof(rdb_stack)], OS_PRIORITY_RMON);
...
_Bool rdb_check(void) { return rdb_active; }
