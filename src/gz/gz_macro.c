...
static int pause_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int advance_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int record_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_THINK) {
...
static int play_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_THINK) {
        menu_switch_set(item, gz.movie_state == MOVIE_PLAYING);
    } else if (reason == MENU_CALLBACK_CHANGED) {
        if (gz.movie_state != MOVIE_PLAYING && gz.movie_frame == gz.movie_input.size) {
            gz_movie_rewind();
        }
        command_playmacro();
    }
...
static int rewind_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_CHANGED) {
        gz_movie_rewind();
    }
    return 0;
...
static int trim_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_CHANGED) {
        vector_erase(&gz.movie_input, gz.movie_frame, gz.movie_input.size - gz.movie_frame);
        vector_erase(&gz.movie_seed, gz.movie_seed_pos, gz.movie_seed.size - gz.movie_seed_pos);
        vector_erase(&gz.movie_oca_input, gz.movie_oca_input_pos, gz.movie_oca_input.size - gz.movie_oca_input_pos);
        vector_erase(&gz.movie_oca_sync, gz.movie_oca_sync_pos, gz.movie_oca_sync.size - gz.movie_oca_sync_pos);
        vector_erase(&gz.movie_room_load, gz.movie_room_load_pos, gz.movie_room_load.size - gz.movie_room_load_pos);
...
static int movie_pos_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int do_import_macro(const char* path, void* data) {
    ...
    if (f != -1) {
        ...
        if (!vector_reserve(&gz.movie_input, n_input) || !vector_reserve(&gz.movie_seed, n_seed)) {
            err_str = s_memory;
            goto error;
        }
        gz_movie_rewind();
        ...
        if (lseek(f, 0, SEEK_CUR) < st.st_size) {
            ...
            if (!vector_reserve(&gz.movie_oca_input, n_oca_input) || !vector_reserve(&gz.movie_oca_sync, n_oca_sync) ||
...
static void import_macro_proc(struct menu_item* item, void* data) {
    menu_get_file(gz.menu_main, GETFILE_LOAD, NULL, ".gzm", do_import_macro, NULL);
}

static void export_macro_proc(struct menu_item* item, void* data) {
    menu_get_file(gz.menu_main, GETFILE_SAVE, "macro", ".gzm", do_export_macro, NULL);
}
...
static void load_state_proc(struct menu_item* item, void* data) { command_loadstate(); }

static void save_state_proc(struct menu_item* item, void* data) { command_savestate(); }
...
static int do_import_state(const char* path, void* data) {
    ...
    if (f != -1) {
        ...
        if (read(f, state, st.st_size) != st.st_size) {
            err_str = strerror(errno);
        } else if (state->z64_version != Z64_VERSION || state->state_version < SETTINGS_STATE_MIN_VER) {
            err_str = s_version;
...
static void import_state_proc(struct menu_item* item, void* data) {
    menu_get_file(gz.menu_main, GETFILE_LOAD, NULL, ".gzs", do_import_state, NULL);
}
...
static void export_state_proc(struct menu_item* item, void* data) {
    if (gz.state_buf[gz.state_slot]) {
        struct state_meta* state = gz.state_buf[gz.state_slot];
        char defname[32];
        snprintf(defname, sizeof(defname), "000-%s", zu_scene_info[state->scene_idx].scene_name);
        menu_get_file(gz.menu_main, GETFILE_SAVE_PREFIX_INC, defname, ".gzs", do_export_state, NULL);
    }
}

static int state_info_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    ...
    if (state) {
        gfx_printf(font, x + cw * 2, y, "%s", zu_scene_info[state->scene_idx].scene_name);
        int cx = 11;
        gfx_printf(font, x + cw * cx, y + ch * 2, "%" PRIu32 "kb", state->size / 1024);
        if (state->movie_frame != -1) {
            ...
            struct gfx_sprite sprite = {
                t_macro, 2, x + cw * cx + (cw - w) / 2, y + ch * 2 - (gfx_font_xheight(font) + h + 1) / 2, 1.f, 1.f,
            };
...
static int hack_oca_input_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int hack_oca_sync_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int hack_room_load_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int wiivc_cam_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int gc_oob_chu_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int vcont_enable_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    int port = (int)data;
    if (reason == MENU_CALLBACK_CHANGED) {
...
static int vcont_plugged_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    int port = (int)data;
    if (reason == MENU_CALLBACK_CHANGED) {
...
static int vcont_joy_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    int port = (int)data / 2;
    int axis = (int)data % 2;
...
static int vcont_button_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int byte_ztarget_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
struct menu* gz_macro_menu(void) {
    ...
    item = menu_add_switch(&menu, 0, 2, t_macro, 0, 0xFF0000, t_macro, 0, 0xFFFFFF, 1.f, 0, pause_switch_proc, NULL);
    item->tooltip = "pause";
    item = menu_add_switch(&menu, 3, 2, t_macro, 1, 0xFFFFFF, t_macro, 1, 0xFFFFFF, 1.f, 0, advance_switch_proc, NULL);
    item->tooltip = "frame advance";
    item = menu_add_switch(&menu, 0, 4, t_macro, 2, 0xFF0000, t_macro, 2, 0xFFFFFF, 1.f, 0, record_switch_proc, NULL);
    item->tooltip = "record macro";
    item = menu_add_switch(&menu, 3, 4, t_macro, 3, 0xFF0000, t_macro, 3, 0xFFFFFF, 1.f, 0, play_switch_proc, NULL);
    item->tooltip = "play macro";
    item = menu_add_switch(&menu, 6, 4, t_macro, 4, 0xFFFFFF, t_macro, 4, 0xFFFFFF, 1.f, 0, rewind_switch_proc, NULL);
    item->tooltip = "rewind macro";
    item = menu_add_switch(&menu, 9, 4, t_macro, 5, 0xFFFFFF, t_macro, 5, 0xFFFFFF, 1.f, 0, trim_switch_proc, NULL);
    item->tooltip = "trim macro";
    item = menu_add_intinput(&menu, 12, 4, 10, 6, movie_pos_proc, NULL);
    item->tooltip = "macro frame";
    menu_add_watch(&menu, 19, 4, (uint32_t)&gz.movie_input.size, WATCH_TYPE_U32);
    item = menu_add_button_icon(&menu, 0, 6, t_save, 0, 0xFFFFFF, import_macro_proc, NULL);
    item->tooltip = "import macro";
    item = menu_add_button_icon(&menu, 3, 6, t_save, 1, 0xFFFFFF, export_macro_proc, NULL);
    item->tooltip = "export macro";
    /* create state controls */
    menu_add_button_icon(&menu, 0, 8, t_arrow, 3, 0xFFFFFF, prev_state_proc, NULL);
    menu_add_button_icon(&menu, 3, 8, t_arrow, 2, 0xFFFFFF, next_state_proc, NULL);
    menu_add_static_custom(&menu, 6, 8, state_info_draw_proc, NULL, 0xC0C0C0);
    item = menu_add_button_icon(&menu, 0, 10, t_save, 2, 0xFFFFFF, load_state_proc, NULL);
    item->tooltip = "load state";
    item = menu_add_button_icon(&menu, 3, 10, t_save, 3, 0xFFFFFF, save_state_proc, NULL);
    item->tooltip = "save state";
    item = menu_add_button_icon(&menu, 6, 10, t_save, 4, 0xFFFFFF, clear_state_proc, NULL);
    item->tooltip = "clear state";
    item = menu_add_button_icon(&menu, 9, 10, t_save, 0, 0xFFFFFF, import_state_proc, NULL);
    item->tooltip = "import state";
    item = menu_add_button_icon(&menu, 12, 10, t_save, 1, 0xFFFFFF, export_state_proc, NULL);
    item->tooltip = "export state";
    /* create movie controls */
    item = menu_add_button_icon(&menu, 0, 13, t_movie, 0, 0xFFFFFF, quick_record_proc, NULL);
    item->tooltip = "quick record movie";
    item = menu_add_button_icon(&menu, 3, 13, t_movie, 1, 0xFFFFFF, quick_play_proc, NULL);
    item->tooltip = "quick play movie";
    /* display rerecord count */
    menu_add_static(&menu, 0, 15, "rerecords:", 0xC0C0C0);
    menu_add_watch(&menu, 11, 15, (uint32_t)&gz.movie_rerecords, WATCH_TYPE_U32);
    ...
    menu_settings.selector = menu_add_submenu(&menu_settings, 0, 0, NULL, "return");
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        menu_add_checkbox(&menu_vcont, 14, 1 + i * 4, vcont_enable_proc, (void*)i);

        menu_add_static(&menu_vcont, 2, 2 + i * 4, "plugged in", 0xC0C0C0);
        menu_add_checkbox(&menu_vcont, 14, 2 + i * 4, vcont_plugged_proc, (void*)i);

        menu_add_static(&menu_vcont, 2, 3 + i * 4, "joystick", 0xC0C0C0);
        menu_add_intinput(&menu_vcont, 14, 3 + i * 4, -10, 4, vcont_joy_proc, (void*)(i * 2 + 0));
        menu_add_intinput(&menu_vcont, 19, 3 + i * 4, -10, 4, vcont_joy_proc, (void*)(i * 2 + 1));
        ...
        for (int j = 0; j < sizeof(buttons) / sizeof(*buttons); ++j) {
            int b = buttons[j];
            item = menu_add_switch(&menu_vcont, 14, 4 + i * 4, t_buttons, b, input_button_color[b], t_buttons, b,
                                   0x808080, 1.f, 0, vcont_button_proc, (void*)(i * 16 + b));
            item->pxoffset = j * 10;
        }
    }
