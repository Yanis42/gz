...
void* zu_zseg_locate(void* ptr) { return zu_seg_locate(&z64_stab, ptr); }
...
void* zu_zseg_relocate(void* p_ptr) { return zu_seg_relocate(p_ptr, &z64_stab); }
...
void zu_getfile(uint32_t vrom_addr, void* dram_addr, size_t size) {
    ...
    z64_getfile_t f = {
        vrom_addr, dram_addr, size, NULL, 0, 0, &notify_mq, 0,
    };
...
void zu_getfile_idx(int file_idx, void* dram_addr) {
    ...
    osCreateMesgQueue(&notify_mq, &notify_m, 1);
    z64_getfile_t f = {
        file->vrom_start, dram_addr, file->vrom_end - file->vrom_start, NULL, 0, 0, &notify_mq, 0,
    };
    osSendMesg(&z64_file_mq, &f, OS_MESG_NOBLOCK);
    osRecvMesg(&notify_mq, NULL, OS_MESG_BLOCK);
}
...
void zu_scene_rooms(const void* scene, struct zu_file* ftab, int ftab_size, int* n_rooms, const z64_stab_t* stab) {
    if (n_rooms) {
        *n_rooms = 0;
    }
...
void zu_room_mesh(const void* room, struct zu_mesh* mesh, const z64_stab_t* stab) {
...
void zu_vlist_init(struct zu_vlist* vlist) { vector_init(&vlist->v, sizeof(Vtx*)); }

void zu_vlist_add_dl(struct zu_vlist* vlist, const z64_stab_t* stab, const Gfx* dl) {
...
void zu_vlist_destroy(struct zu_vlist* vlist) { vector_destroy(&vlist->v); }
...
_Noreturn void zu_reset(void) {
    ...
    uint32_t imem[] = {
        MIPS_LUI(MIPS_T5, 0xBFC0),           MIPS_LW(MIPS_T0, 0x07FC, MIPS_T5),   MIPS_ADDIU(MIPS_T5, MIPS_T5, 0x07C0),
        MIPS_ANDI(MIPS_T0, MIPS_T0, 0x0080), MIPS_BNEL(MIPS_T0, MIPS_R0, -4 * 4), MIPS_LUI(MIPS_T5, 0xBFC0),
        MIPS_LW(MIPS_T0, 0x0024, MIPS_T5),   MIPS_LUI(MIPS_T3, 0xB000),
    };
...
void zu_execute_game(int16_t entrance_index, uint16_t cutscene_index) {
    if (entrance_index != z64_file.entrance_index || cutscene_index != z64_file.cutscene_index) {
...
_Bool zu_in_game(void) { return (uint32_t)z64_ctxt.state_dtor == z64_state_ovl_tab[3].vram_dtor; }

void zu_audio_cmd(uint32_t cmd) { z64_audio_cmd_buf[z64_audio_cmd_write_pos++] = cmd; }

void zu_set_event_flag(int flag_index) { z64_file.event_chk_inf[flag_index / 0x10] |= (1 << (flag_index % 0x10)); }

void zu_clear_event_flag(int flag_index) { z64_file.event_chk_inf[flag_index / 0x10] &= ~(1 << (flag_index % 0x10)); }
...
static z64_gbi_lights_t* gen_lighting(Gfx** p_gfx_d) {
    ...
    /* fill light */
    for (z64_light_node_t* light_node = z64_game.lighting.light_list; light_node; light_node = light_node->next) {
...
static void set_lighting(Gfx** p_gfx_p, z64_gbi_lights_t* gbi_lights) {
    ...
    gSPLight((*p_gfx_p)++, &gbi_lights->lites.a, gbi_lights->numlights + 1);
}
...
void zu_set_lighting_ext(Gfx** p_gfx_p, Gfx** p_gfx_d) { set_lighting(p_gfx_p, gen_lighting(p_gfx_d)); }
