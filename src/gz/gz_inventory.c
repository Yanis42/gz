...
static struct equipment_switch equipment_list[] = {
    {0x0001, Z64_ITEM_KOKIRI_SWORD},        {0x0002, Z64_ITEM_MASTER_SWORD},  {0x0004, Z64_ITEM_BIGGORON_SWORD},
    {0x0010, Z64_ITEM_DEKU_SHIELD},         {0x0020, Z64_ITEM_HYLIAN_SHIELD}, {0x0040, Z64_ITEM_MIRROR_SHIELD},
    {0x0100, Z64_ITEM_KOKIRI_TUNIC},        {0x0200, Z64_ITEM_GORON_TUNIC},   {0x0400, Z64_ITEM_ZORA_TUNIC},
    {0x1000, Z64_ITEM_KOKIRI_BOOTS},        {0x2000, Z64_ITEM_IRON_BOOTS},    {0x4000, Z64_ITEM_HOVER_BOOTS},
...
static int8_t bottle_options[] = {
    Z64_ITEM_NULL,  Z64_ITEM_BOTTLE,  Z64_ITEM_RED_POTION, Z64_ITEM_GREEN_POTION, Z64_ITEM_BLUE_POTION,
    Z64_ITEM_FAIRY, Z64_ITEM_FISH,    Z64_ITEM_MILK,       Z64_ITEM_LETTER,       Z64_ITEM_BLUE_FIRE,
    Z64_ITEM_BUG,   Z64_ITEM_BIG_POE, Z64_ITEM_HALF_MILK,  Z64_ITEM_POE,
};

static int8_t adult_trade_options[] = {
    Z64_ITEM_NULL,         Z64_ITEM_POCKET_EGG,   Z64_ITEM_POCKET_CUCCO, Z64_ITEM_COJIRO,
    Z64_ITEM_ODD_MUSHROOM, Z64_ITEM_ODD_POTION,   Z64_ITEM_POACHERS_SAW, Z64_ITEM_BROKEN_GORONS_SWORD,
    Z64_ITEM_PRESCRIPTION, Z64_ITEM_EYEBALL_FROG, Z64_ITEM_EYE_DROPS,    Z64_ITEM_CLAIM_CHECK,
};

static int8_t child_trade_options[] = {
    Z64_ITEM_NULL,        Z64_ITEM_WEIRD_EGG,     Z64_ITEM_CHICKEN,    Z64_ITEM_ZELDAS_LETTER, Z64_ITEM_KEATON_MASK,
    Z64_ITEM_SKULL_MASK,  Z64_ITEM_SPOOKY_MASK,   Z64_ITEM_BUNNY_HOOD, Z64_ITEM_GORON_MASK,    Z64_ITEM_ZORA_MASK,
    Z64_ITEM_GERUDO_MASK, Z64_ITEM_MASK_OF_TRUTH, Z64_ITEM_SOLD_OUT,
};

static struct quest_item_switch quest_item_list[] = {
    {0x00000020, 5}, {0x00000001, 0}, {0x00000002, 1}, {0x00000004, 2}, {0x00000008, 3},  {0x00000010, 4},
    {0x00040000, 6}, {0x00080000, 7}, {0x00100000, 8}, {0x00200000, 9}, {0x00400000, 10}, {0x00800000, 11},
};

static struct song_switch song_list[] = {
    {0x00001000, 0xFFFFFF, "zelda's lullaby"},    {0x00002000, 0xFFFFFF, "epona's song"},
    {0x00004000, 0xFFFFFF, "saria's song"},       {0x00008000, 0xFFFFFF, "sun's song"},
    {0x00010000, 0xFFFFFF, "song of time"},       {0x00020000, 0xFFFFFF, "song of storms"},
    {0x00000040, 0x96FF64, "minuet of forest"},   {0x00000080, 0xFF5028, "bolero of fire"},
    {0x00000100, 0x6496FF, "serenade of water"},  {0x00000200, 0xFFA000, "requiem of spirit"},
    {0x00000400, 0xFF64FF, "nocturne of shadow"}, {0x00000800, 0xFFF064, "prelude of light"},
};
...
static int byte_mod_indirect_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    uint8_t** pp = data;
    if (reason == MENU_CALLBACK_THINK_INACTIVE) {
...
static int byte_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int equipment_item_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    struct equipment_item_data* d = data;
    if (reason == MENU_CALLBACK_CHANGED) {
        if (d->value == Z64_ITEM_NULL) {
            z64_file.equipment_items &= ~(d->mask << d->shift);
        } else {
            z64_file.equipment_items =
                (z64_file.equipment_items & ~(d->mask << d->shift)) | ((d->value - d->start + 1) << d->shift);
        }
    } else if (reason == MENU_CALLBACK_THINK) {
...
static int capacity_item_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    ...
    if (reason == MENU_CALLBACK_CHANGED) {
        ...
        z64_file.equipment_items = (z64_file.equipment_items & ~(0b111 << d->shift)) | (v << d->shift);
    } else if (reason == MENU_CALLBACK_THINK) {
...
static int equipment_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int bgs_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
        z64_file.bgs_flag = 1;
...
static int item_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int double_defense_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int magic_acquired_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int magic_capacity_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_THINK_INACTIVE) {
...
static int dungeon_option_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
static int quest_item_switch_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
...
struct menu* gz_inventory_menu(void) {
    ...
    for (int i = 0; i < n_eq_items; ++i) {
        ...
        item = item_option_create(&equipment, 0, 2, option->length + 1, item_list, &data->value, NULL, t_ab, 1, NULL, 0,
                                  0xFFFFFF, .5f, .5625f, 0.f, equipment_item_proc, data);
    ...
    for (int i = 0; i < n_cap_item; ++i) {
        ...
        for (int j = 0; j < n_tiles; ++j) {
            ...
            gfx_texture_copy_tile(tex, j, t_am, option->amount_tiles[8 - n_tiles + j], 1);
        }
        ...
        item = item_option_create(&equipment, 0, 2, 8, option->multi_tile ? multi_options : options, &data->value, tex,
                                  t_ab, 1, NULL, 0, 0xFFFFFF, .5f, .5625f, 0.f, capacity_item_proc, data);
        item->pxoffset = option->x * 18;
        item->pyoffset = option->y * 18;
        item->tooltip = option->tooltip;
    }
    ...
    for (int i = 0; i < n_eq; ++i) {
        item = menu_add_switch(&equipment, 0, 2, t_item, equipment_list[i].item_id, 0xFFFFFF, t_item_gray,
                               equipment_list[i].item_id, 0xFFFFFF, .5f, 1, equipment_switch_proc, &equipment_list[i]);
        item->pxoffset = 20 + i % 3 * 18;
        item->pyoffset = i / 3 * 18;
    }
    /* giant's knife / biggoron's sword switch */
    item = menu_add_switch(&equipment, 0, 2, t_item, Z64_ITEM_BIGGORON_SWORD, 0xFFFFFF, t_item_gray,
                           Z64_ITEM_BIGGORON_SWORD, 0xFFFFFF, .5f, 1, bgs_switch_proc, NULL);
    item->pxoffset = 20 + 1 * 18;
    item->pyoffset = 4 * 18;
    /* normal items */
    int n_item = sizeof(item_list) / sizeof(*item_list);
    for (int i = 0; i < n_item; ++i) {
        item = menu_add_switch(&equipment, 0, 2, t_item, item_list[i].item_id, 0xFFFFFF, t_item_gray,
                               item_list[i].item_id, 0xFFFFFF, .5f, 1, item_switch_proc, &item_list[i]);
    ...
    for (int i = 0; i < 4; ++i) {
        item =
            item_option_create(&equipment, 0, 2, n_bottle_opt, bottle_options, &z64_file.items[Z64_SLOT_BOTTLE_1 + i],
                               NULL, t_ab, 1, NULL, 0, 0xFFFFFF, .5f, .5625f, 0.f, NULL, NULL);
    ...
    item = item_option_create(&equipment, 0, 2, n_adult_opt, adult_trade_options, &z64_file.items[Z64_SLOT_ADULT_TRADE],
                              NULL, t_ab, 1, NULL, 0, 0xFFFFFF, .5f, .5625f, 0.f, NULL, NULL);
    ...
    item = item_option_create(&equipment, 0, 2, n_child_opt, child_trade_options, &z64_file.items[Z64_SLOT_CHILD_TRADE],
                              NULL, t_ab, 1, NULL, 0, 0xFFFFFF, .5f, .5625f, 0.f, NULL, NULL);
    ...
    menu_add_intinput(&quest, 14, 1, 16, 4, halfword_mod_proc, &z64_file.energy_capacity);
    menu_add_static(&quest, 0, 2, "defense", 0xC0C0C0);
    menu_add_checkbox(&quest, 14, 2, double_defense_proc, NULL);
    menu_add_intinput(&quest, 16, 2, 16, 2, byte_mod_proc, &z64_file.defense_hearts);
    /* magic options */
    ...
    menu_add_intinput(&quest, 14, 5, 10, 3, halfword_mod_proc, &z64_file.gs_tokens);
    menu_add_static(&quest, 0, 6, "heart pieces", 0xC0C0C0);
    menu_add_intinput(&quest, 14, 6, 16, 2, byte_mod_proc, &z64_file.quest_items);
    /* dungeon items */
    ...
    menu_add_intinput(&quest, 14, 8, 16, 2, byte_mod_indirect_proc, &di.small_keys);
    item = menu_add_switch(&quest, 0, 10, t_item_24, 14, 0xFFFFFF, t_item_24, 14, 0x606060, 2.f / 3.f, 1,
                           byte_switch_proc, &di.boss_key);
    item->pxoffset = 5 * 18 + 20;
    item = menu_add_switch(&quest, 0, 10, t_item_24, 15, 0xFFFFFF, t_item_24, 15, 0x606060, 2.f / 3.f, 1,
                           byte_switch_proc, &di.compass);
    item->pxoffset = 5 * 18 + 20;
    item->pyoffset = 1 * 18;
    item = menu_add_switch(&quest, 0, 10, t_item_24, 16, 0xFFFFFF, t_item_24, 16, 0x606060, 2.f / 3.f, 1,
    ...
    for (int i = 0; i < n_quest_item; ++i) {
        ...
        item = menu_add_switch(&quest, 0, 10, t_item_24, d->tile, 0xFFFFFF, t_item_24, d->tile, 0x606060, 2.f / 3.f, 1,
    ...
    for (int i = 0; i < n_song; ++i) {
        ...
        uint32_t color_off = ((r * 0x60 / 0xFF) << 16) | ((g * 0x60 / 0xFF) << 8) | ((b * 0x60 / 0xFF) << 0);
        item = menu_add_switch(&quest, 0, 10, t_note, 0, d->color, t_note, 0, color_off, 2.f / 3.f, 1,
                               quest_item_switch_proc, (void*)d->mask);
        item->pxoffset = i % 6 * 18;
    ...
    menu_add_static_icon(&amount, 5, 4, t_item, Z64_ITEM_SLINGSHOT, 0xFFFFFF, .5f);
    menu_add_static_icon(&amount, 10, 4, t_item, Z64_ITEM_BOMBCHU, 0xFFFFFF, .5f);
    ...
    menu_add_static_icon(&amount, 0, 8, t_item, Z64_ITEM_BIGGORON_SWORD, 0xFFFFFF, .5f);
    menu_add_static_icon(&amount, 10, 8, t_rupee, 0, 0xC8FF64, 1.f);
    menu_add_intinput(&amount, 2, 2, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_STICK]);
    menu_add_intinput(&amount, 7, 2, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_NUT]);
    menu_add_intinput(&amount, 12, 2, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_BOMB]);
    menu_add_intinput(&amount, 2, 4, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_BOW]);
    menu_add_intinput(&amount, 7, 4, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_SLINGSHOT]);
    menu_add_intinput(&amount, 12, 4, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_BOMBCHU]);
    menu_add_intinput(&amount, 2, 6, 10, 2, byte_mod_proc, &z64_file.ammo[Z64_SLOT_BEANS]);
    menu_add_intinput(&amount, 7, 6, 16, 2, byte_mod_proc, &z64_file.magic);
    menu_add_intinput(&amount, 12, 6, 16, 4, halfword_mod_proc, &z64_file.energy);
    menu_add_intinput(&amount, 2, 8, 16, 4, halfword_mod_proc, &z64_file.bgs_hits_left);
    menu_add_intinput(&amount, 12, 8, 10, 5, halfword_mod_proc, &z64_file.rupees);
