...
static void call_navi_proc(struct menu_item* item, void* data) { z64_file.navi_timer = 0x025A; }
...
static void set_time_proc(struct menu_item* item, void* data) { gz.target_day_time = (uint32_t)data; }

static void set_epona_flag_proc(struct menu_item* item, void* data) { zu_set_event_flag(0x18); }

static void clear_epona_flag_proc(struct menu_item* item, void* data) { zu_clear_event_flag(0x18); }
...
struct menu* gz_file_menu(void) {
    ...
    menu_add_button_icon(&menu, 17, 4, t_daytime, 0, 0xFFC800, set_time_proc, (void*)0x4AB0);
    menu_add_button_icon(&menu, 19, 4, t_daytime, 1, 0xA0A0E0, set_time_proc, (void*)0xC010);
    menu_add_intinput(&menu, 21, 4, 16, 4, halfword_mod_proc, &z64_file.day_time);

    /* create flag controls */
    struct gfx_texture* t_check = resource_get(RES_ICON_CHECK);
    menu_add_static(&menu, 0, 5, "epona freed", 0xC0C0C0);
    menu_add_button_icon(&menu, 17, 5, t_check, 0, 0x00FF00, set_epona_flag_proc, NULL);
    menu_add_button_icon(&menu, 19, 5, t_check, 1, 0xFF0000, clear_epona_flag_proc, NULL);
    menu_add_static(&menu, 0, 6, "carpenters freed", 0xC0C0C0);
    menu_add_button_icon(&menu, 17, 6, t_check, 0, 0x00FF00, set_carpenter_flags_proc, NULL);
    menu_add_button_icon(&menu, 19, 6, t_check, 1, 0xFF0000, clear_carpenter_flags_proc, NULL);
    menu_add_static(&menu, 0, 7, "intro cutscenes", 0xC0C0C0);
    menu_add_button_icon(&menu, 17, 7, t_check, 0, 0x00FF00, set_intro_flags_proc, NULL);
    menu_add_button_icon(&menu, 19, 7, t_check, 1, 0xFF0000, clear_intro_flags_proc, NULL);
    menu_add_static(&menu, 0, 8, "rewards obtained", 0xC0C0C0);
    menu_add_button_icon(&menu, 17, 8, t_check, 0, 0x00FF00, set_reward_flags_proc, NULL);
    menu_add_button_icon(&menu, 19, 8, t_check, 1, 0xFF0000, clear_reward_flags_proc, NULL);

    /* create timer controls */
    menu_add_static(&menu, 0, 9, "timer 1", 0xC0C0C0);
    menu_add_intinput(&menu, 17, 9, 10, 5, halfword_mod_proc, &z64_file.timer_1_value);
    ...
    menu_add_intinput(&menu, 17, 10, 10, 5, halfword_mod_proc, &z64_file.timer_2_value);
