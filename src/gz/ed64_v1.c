...
static inline uint32_t reg_rd(int reg) { return __pi_read_raw((uint32_t)&REGS_PTR[reg]); }

static inline void reg_wr(int reg, uint32_t dat) { return __pi_write_raw((uint32_t)&REGS_PTR[reg], dat); }
...
static inline uint8_t spi_rx(void) { return spi_io(0xFF); }
...
static int sd_spi_io(int dat) { return spi_io(dat); }
...
static int disk_init(void) { return sd_init(&sd_host); }

static int disk_read(size_t lba, size_t n_blocks, void* dst) { return sd_read(&sd_host, lba, dst, n_blocks); }

static int disk_write(size_t lba, size_t n_blocks, const void* src) { return sd_write(&sd_host, lba, src, n_blocks); }
