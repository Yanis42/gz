...
_Static_assert(SETTINGS_PROFILE_MAX != 0, "settings data size exceeds sram capacity");

static _Alignas(16) struct settings settings_store;
...
void settings_load_default(void) {
    ...
    d->binds[COMMAND_RESETTIMER] = bind_make(3, BUTTON_R, BUTTON_B, BUTTON_D_LEFT);
...
void settings_save(int profile) {
    ...
    *checksum = settings_checksum_compute(&settings_store);
    zu_sram_write(&settings_store, SETTINGS_ADDRESS + SETTINGS_PADSIZE * profile, sizeof(settings_store));
}

_Bool settings_load(int profile) {
    _Alignas(16) struct settings settings_temp;
    zu_sram_read(&settings_temp, SETTINGS_ADDRESS + SETTINGS_PADSIZE * profile, sizeof(settings_temp));
