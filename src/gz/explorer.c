...
static void draw_crosshair(struct menu_item* item) {
    ...
    gDisplayListAppend(&data->gfx.poly_xlu.p,
                       /* load mesh matricies */
                       gsSPDisplayList(&init_xlu), gsSPMatrix(p_latx_mtx, G_MTX_MODELVIEW | G_MTX_LOAD),
                       gsSPVertex(&lat_mesh, 4, 0), gsSPMatrix(p_vert_mtx, G_MTX_MODELVIEW | G_MTX_LOAD),
                       gsSPVertex(&vert_mesh, 4, 4), gsSPMatrix(p_latz_mtx, G_MTX_MODELVIEW | G_MTX_LOAD),
    ...
    gDisplayListAppend(&data->gfx.poly_xlu.p, gsDPSetPrimColor(0, 0, 0x00, 0x00, 0xFF, 0x40),
                       gsSP2Triangles(8, 9, 10, 0, 10, 9, 11, 0), gsDPPipeSync(),
                       gsDPSetPrimColor(0, 0, 0xFF, 0x00, 0x00, 0x40), gsSP2Triangles(0, 1, 2, 0, 2, 1, 3, 0),
                       gsDPPipeSync(), gsDPSetPrimColor(0, 0, 0x00, 0xFF, 0x00, 0x40),
                       gsSP2Triangles(4, 5, 6, 0, 6, 5, 7, 0), );
}

static int enter_proc(struct menu_item* item, enum menu_switch_reason reason) {
    input_reserve(BUTTON_D_UP | BUTTON_D_DOWN | BUTTON_D_LEFT | BUTTON_D_RIGHT | BUTTON_Z);
...
static int leave_proc(struct menu_item* item, enum menu_switch_reason reason) {
    input_free(BUTTON_D_UP | BUTTON_D_DOWN | BUTTON_D_LEFT | BUTTON_D_RIGHT | BUTTON_Z);
...
static int think_proc(struct menu_item* item) {
    ...
    if ((data->scene_index != data->scene_next || data->room_index != data->room_next) && data->state == STATE_RENDER) {
...
static int draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    ...
    struct item_data* data = item->data;
    /* handle input */
    if (!input_bind_held(COMMAND_PREVROOM) && !input_bind_held(COMMAND_NEXTROOM)) {
    ...
    if (data->state == STATE_LOAD) {
        ...
        zu_scene_rooms(zu_sr_header(data->scene_file, z64_file.scene_setup_index, &stab), room_files, 0x100,
                       &data->n_rooms, &stab);
        /* load room */
        if (data->room_index != data->room_next || !data->room_file) {
            ...
            /* populate mesh */
            zu_room_mesh(zu_sr_header(data->room_file, z64_file.scene_setup_index, &stab), &data->room_mesh, &stab);
            ...
            for (int i = 0; i < ZU_MESH_TYPES; ++i) {
                for (int j = 0; j < data->room_mesh.all[i].size; ++j) {
                    zu_vlist_add_dl(&vlist, &stab, zu_seg_locate(&stab, data->room_mesh.all[i].dlists[j]));
                }
            }
    ...
    if (data->state == STATE_RENDER && data->room_file) {
        ...
        static Gfx init_rcp[] = {
            ...
            gsDPSetScissor(G_SC_NON_INTERLACE, 32, 32, Z64_SCREEN_WIDTH - 32, Z64_SCREEN_HEIGHT - 32),
            gsSPEndDisplayList(),
        };
        gDisplayListAppend(
            &data->gfx.poly_opa.p, gsDPPipeSync(), gsDPSetCycleType(G_CYC_FILL),
            ...
            gsDPFillRectangle(0, 0, Z64_SCREEN_WIDTH - 1, Z64_SCREEN_HEIGHT - 1), gsDPPipeSync(),
            gsDPSetColorImage(G_IM_FMT_RGBA, G_IM_SIZ_16b, Z64_SCREEN_WIDTH, ZU_MAKE_SEG(Z64_SEG_CIMG, 0)),
            gsDPSetDepthImage(zbuf), gsSPSegment(Z64_SEG_SCENE, data->scene_file),
            gsSPSegment(Z64_SEG_ROOM, data->room_file), gsSPDisplayList(init_rcp), );
        /* create projection matrix */
        {
            Mtx m;
            MtxF mf;
            MtxF mt;
            guPerspectiveF(&mf, NULL, atanf(2.f), (float)Z64_SCREEN_WIDTH / (float)Z64_SCREEN_HEIGHT, 50.f, 5000.f,
                           1.f);
            {
            ...
            guMtxF2L(&mf, &m);
            gSPMatrix(data->gfx.poly_opa.p++, gDisplayListData(&data->gfx.poly_opa.d, m),
                      G_MTX_PROJECTION | G_MTX_LOAD);
            gSPMatrix(data->gfx.poly_xlu.p++, gDisplayListData(&data->gfx.poly_xlu.d, m),
                      G_MTX_PROJECTION | G_MTX_LOAD);
        }
        /* create modelview matrix */
        {
            Mtx m;
            guMtxIdent(&m);
            gSPMatrix(data->gfx.poly_opa.p++, gDisplayListData(&data->gfx.poly_opa.d, m), G_MTX_MODELVIEW | G_MTX_LOAD);
            gSPMatrix(data->gfx.poly_xlu.p++, gDisplayListData(&data->gfx.poly_xlu.d, m), G_MTX_MODELVIEW | G_MTX_LOAD);
        }
        /* configure lights */
        zu_gfx_inject(&data->gfx);
        zu_set_lighting();
        /* execute scene config */
        z64_scene_config_table[z64_scene_table[z64_game.scene_index].scene_config](&z64_game);
        zu_gfx_restore(&data->gfx);
        /* draw scene */
        for (int i = 0; i < ZU_MESH_TYPES; ++i) {
            for (int j = 0; j < data->room_mesh.all[i].size; ++j) {
                if (i == ZU_MESH_OPA || i == ZU_MESH_NEAR) {
                    gSPDisplayList(data->gfx.poly_opa.p++, data->room_mesh.all[i].dlists[j]);
                } else if (i == ZU_MESH_XLU || i == ZU_MESH_FAR) {
                    gSPDisplayList(data->gfx.poly_xlu.p++, data->room_mesh.all[i].dlists[j]);
                }
            }
        }
        /* draw actors */
        ...
        gfx_mode_init();
        /* draw info */
        gfx_mode_set(GFX_MODE_COLOR, GPACK_RGBA8888(0xC0, 0xC0, 0xC0, draw_params->alpha));
        gfx_printf(draw_params->font, 36, 44, "scene %i", data->scene_index);
        gfx_printf(draw_params->font, 36, 44 + menu_get_cell_height(item->owner, 1), "room  %i", data->room_index);
    }
...
static int activate_proc(struct menu_item* item) {
    ...
    z64_link.common.rot_2.y = z64_link.target_yaw = 0x8000 + data->yaw * 0x8000 / M_PI;
