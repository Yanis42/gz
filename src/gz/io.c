...
static unsigned int clock_freq_dflt(void) { return OS_CPU_COUNTER; }

static void cpu_reset_dflt(void) { return zu_reset(); }

static struct iodev* current_dev;

int io_init(void) {
    struct iodev* devs[] = {
        &homeboy_iodev, &everdrive64_x, &everdrive64_v2, &everdrive64_v1, &sc64,
    };
