...
void gfx_mode_configure(enum gfx_mode mode, uint64_t value) { gfx_modes[mode] = value; }
...
void gfx_sprite_draw(const struct gfx_sprite* sprite) {
    ...
    if (gfx_modes[GFX_MODE_DROPSHADOW]) {
        ...
        gSPScisTextureRectangle(gfx_disp_p++, ulx + qs102(1), uly + qs102(1), lrx + qs102(1), lry + qs102(1),
                                G_TX_RENDERTILE, uls, ult, dsdx, dtdy);
        gfx_mode_pop(GFX_MODE_COLOR);
    }
    gfx_sync();
    gSPScisTextureRectangle(gfx_disp_p++, ulx, uly, lrx, lry, G_TX_RENDERTILE, uls, ult, dsdx, dtdy);
    gfx_synced = 0;
}

int gfx_font_xheight(const struct gfx_font* font) { return font->baseline - font->median; }
