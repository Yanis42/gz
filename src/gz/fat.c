...
static void cache_dirty(struct fat* fat, int index) { fat->cache[index].dirty = 1; }

static void cache_inval(struct fat* fat, int index) { fat->cache[index].valid = 0; }
...
struct fat_entry* fat_path_dir(struct fat_path* fp) { return list_prev(fp->ent_list.last); }
