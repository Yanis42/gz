...
static void wheel_draw_item(struct item_data* data, int item_id, float rot) {
    static Vtx mesh[] = {
    ...
    /* build dlist */
    gfx_disp(gsSPMatrix(p_mtx, G_MTX_MODELVIEW | G_MTX_LOAD), gsSPVertex(&mesh, 4, 0), );
...
static int wheel_draw_proc(struct menu_item* item, struct menu_draw_params* draw_params) {
    ...
    /* create projection matrix */
    {
        ...
        MtxF mf;
        MtxF mt;
        { guPerspectiveF(&mf, NULL, M_PI / 4.f, (float)Z64_SCREEN_WIDTH / (float)Z64_SCREEN_HEIGHT, 1.f, 100.f, 1.f); }
        {
            guTranslateF(&mt, 0.f, 0.f, 1.f - (Z64_SCREEN_WIDTH / 32.f * 2.f + data->n_items / M_PI));
            guMtxCatF(&mt, &mf, &mf);
        }
        guMtxF2L(&mf, &m);
        gfx_disp(gsSPMatrix(gfx_data_append(&m, sizeof(m)), G_MTX_PROJECTION | G_MTX_LOAD));
    }
    /* draw items */
    animate_wheel(data, 1.f / 3.f);
    int n = (lroundf(-data->wheel_rot / (M_PI * 2.f) * data->n_items) + (data->n_items + 3) / 4) % data->n_items;
    if (n < 0) {
        n += data->n_items;
    }
    uint32_t color = -1;
    for (int i = 0; i < data->n_items; ++i) {
        ...
        if (color != c) {
            color = c;
            gfx_disp(gsDPPipeSync(), gsDPSetPrimColor(0, 0, r, g, b, draw_params->alpha));
        }
...
static int wheel_navigate_proc(struct menu_item* item, enum menu_navigation nav) {
    struct item_data* data = item->data;
    int n = 0;
    switch (nav) {
...
struct menu_item* item_option_create(struct menu* menu, int x, int y, int n_items, const int8_t* item_list,
                                     int8_t* value, struct gfx_texture* texture, struct gfx_texture* bg_texture,
                                     int bg_texture_tile, struct gfx_texture* empty_texture, int empty_texture_tile,
                                     uint32_t color, float scale, float bg_scale, float empty_scale,
