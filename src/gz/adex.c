...
static _Bool is_func_tok(struct tok* tok) { return tok->type == TOK_OP && op_ar[tok->op] == 1; }
...
static void stack_init(struct vector* stack) { vector_init(stack, sizeof(struct tok)); }
...
static _Bool stack_push(struct vector* stack, struct tok* tok) { return vector_push_back(stack, 1, tok); }
...
void adex_destroy(struct adex* adex) { vector_destroy(&adex->expr); }
