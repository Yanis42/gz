...
__attribute__((section(".data"))) struct gz gz = {
    .ready = 0,
};
...
static void main_hook(void) {
    ...
    if (settings->cheats & (1 << CHEAT_SEEDS)) {
        ...
        z64_file.ammo[Z64_SLOT_SLINGSHOT] = bullet_bag_capacity[z64_file.bullet_bag];
    }
    ...
    if (settings->cheats & (1 << CHEAT_FREEZETIME)) {
        ...
        if (gz.target_day_time == -1 && z64_day_speed < 0x0190 &&
            (z64_file.day_time == time_advance_1 || z64_file.day_time == time_advance_2)) {
    ...
    if (settings->cheats & (1 << CHEAT_USEITEMS)) {
        memset(&z64_game.if_ctxt.restriction_flags, 0, sizeof(z64_game.if_ctxt.restriction_flags));
    }
    ...
    for (int i = 0; i < COMMAND_MAX; ++i) {
        ...
        switch (command_info[i].activation_type) {
            case CMDACT_HOLD:
                active = input_bind_held(i);
                break;
            case CMDACT_PRESS:
                active = input_bind_pressed(i);
                break;
            case CMDACT_PRESS_ONCE:
                active = input_bind_pressed_raw(i);
                break;
        }
    ...
    if (gz.target_day_time != -1) {
        ...
        if (z64_file.day_time < gz.target_day_time && gz.target_day_time - z64_file.day_time <= speed) {
    ...
    if (settings->bits.pause_display) {
        if (gz.movie_state == MOVIE_RECORDING) {
            ...
            struct gfx_sprite sprite = {
                t, gz.frames_queued == -1 ? 3 : gz.frames_queued == 0 ? 4 : 5, 32, 32, 1.f, 1.f,
            };
            ...
            gfx_printf(font, 32, 48 + ch, "%i / %i", gz.movie_frame, gz.movie_input.size);
        } else if (gz.movie_state == MOVIE_PLAYING) {
            struct gfx_texture* t = resource_get(RES_ICON_PAUSE);
            struct gfx_sprite sprite = {
                t, gz.frames_queued == -1 ? 0 : gz.frames_queued == 0 ? 1 : 2, 32, 32, 1.f, 1.f,
            };
            gfx_mode_set(GFX_MODE_COLOR, GPACK_RGBA8888(0xC0, 0x00, 0x00, alpha));
            gfx_sprite_draw(&sprite);
            gfx_mode_set(GFX_MODE_COLOR, GPACK_RGBA8888(0xC0, 0xC0, 0xC0, alpha));
            ...
            struct gfx_texture* t = resource_get(RES_ICON_PAUSE);
            struct gfx_sprite sprite = {
                t, gz.frames_queued == 0 ? 1 : 2, 32, 32, 1.f, 1.f,
            };
            gfx_mode_set(GFX_MODE_COLOR, GPACK_RGBA8888(0xC0, 0xC0, 0xC0, alpha));
            gfx_sprite_draw(&sprite);
        }
    ...
    if (settings->bits.input_display) {
        ...
        if (gz.movie_state == MOVIE_PLAYING && gz.movie_frame < gz.movie_input.size) {
            ...
            uint16_t mask = BUTTON_L | BUTTON_D_UP | BUTTON_D_DOWN | BUTTON_D_LEFT | BUTTON_D_RIGHT;
            d_pad &= ~mask;
            d_pressed &= ~mask;
            d_released &= ~mask;
        } else {
        ...
        for (int i = 0; i < sizeof(buttons) / sizeof(*buttons); ++i) {
            ...
            int bit = 1 << b;
            int b_alpha;
            if (settings->bits.input_pressrel && ((d_pressed | d_released) & ~d_pad & bit)) {
                b_alpha = 0x7F;
            } else if (d_pad & bit) {
                b_alpha = 0xFF;
            } else {
            ...
            struct gfx_sprite sprite = {
                texture, b, settings->input_display_x + cw * 10 + x, settings->input_display_y + y, 1.f, 1.f,
            };
            gfx_mode_set(GFX_MODE_COLOR, GPACK_RGB24A8(input_button_color[b], b_alpha * alpha / 0xFF));
            gfx_sprite_draw(&sprite);
    ...
    if (settings->bits.lag_counter) {
        int32_t lag_frames = (int32_t)__osViIntrCount + gz.lag_vi_offset - gz.frame_counter;
...
#define STRINGIFY_(S) #S
        ...
        if (splash_time > 0) {
            ...
            gfx_printf(font, Z64_SCREEN_WIDTH - 16 - cw * strlen(url), Z64_SCREEN_HEIGHT - 12 - ch * 2, url);
            if (!logo_texture) {
            ...
            for (int i = 0; i < logo_texture->tiles_y; ++i) {
                struct gfx_sprite logo_sprite = {
                    ...
                    Z64_SCREEN_HEIGHT - 12 - ch * 3 - (logo_texture->tiles_y - i) * logo_texture->tile_height,
                    1.f,
                    1.f,
                };
    ...
    if (gz.frames_queued != 0) {
        ...
        if (z64_ctxt.state_frames != 0) {
            ...
            if (gfx->frame_count_1 & 1) {
                memcpy((void*)&z64_disp[z64_disp_size], (void*)&z64_disp[0], z64_disp_size);
            } else {
                memcpy((void*)&z64_disp[0], (void*)&z64_disp[z64_disp_size], z64_disp_size);
            }
            /* set pointers */
            ...
            gDPSetColorImage(gfx->poly_opa.p++, G_IM_FMT_RGBA, G_IM_SIZ_16b, Z64_SCREEN_WIDTH,
            ...
            gDPSetFillColor(gfx->poly_opa.p++,
                            (GPACK_RGBA5551(0x00, 0x00, 0x00, 0x01) << 16) | GPACK_RGBA5551(0x00, 0x00, 0x00, 0x01));
            gDPFillRectangle(gfx->poly_opa.p++, 0, 0, Z64_SCREEN_WIDTH - 1, Z64_SCREEN_HEIGHT - 1);
...
HOOK void camera_hook(void* camera) {
    ...
    __asm__("sw      $t9, %[camera_func]" : [camera_func] "=m"(camera_func));
...
HOOK void metronome_start_hook(uint16_t sfx_id, z64_xyzf_t* pos, uint8_t token, float* freq_scale, float* vol,
                               int8_t* reverb_add) {
    maybe_init_gp();
    ...
    return z64_Audio_PlaySfxGeneral(sfx_id, pos, token, freq_scale, vol, reverb_add);
}
...
HOOK void bombchu_floor_poly_hook(z64_game_t* game, z64_actor_t* actor, float wall_check_height,
                                  float wall_check_radius, float ceiling_check_height, int32_t flags) {
    maybe_init_gp();
    z64_Actor_UpdateBgCheckInfo(game, actor, wall_check_height, wall_check_radius, ceiling_check_height, flags);
...
static void main_return_proc(struct menu_item* item, void* data) { gz_hide_menu(); }
...
static void init(void) {
    ...
    gz.z_input_mask.pad = BUTTON_L | BUTTON_D_RIGHT | BUTTON_D_LEFT | BUTTON_D_DOWN | BUTTON_D_UP;
    gz.z_input_mask.x = 0;
    gz.z_input_mask.y = 0;
    for (int i = 0; i < 4; ++i) {
