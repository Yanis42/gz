#include "zu.h"

struct zu_scene_info zu_scene_info[] = {
    {
        "deku tree",
        2,
        (const char*[]){
            "from forest",
            "from boss",
        },
    },
    {
        "dodongo's cavern",
        2,
        (const char*[]){
            "from trail",
            "from boss",
        },
    },
    {
        "jabu jabu",
        2,
        (const char*[]){
            "from fountain",
            "from boss",
        },
    },
    {
        "forest temple",
        3,
        (const char*[]){
            "from meadow",
            "from boss",
            "ceiling crush room",
        },
    },
    {
        "fire temple",
        2,
        (const char*[]){
            "from crater",
            "from boss",
        },
    },
    {
        "water temple",
        2,
        (const char*[]){
            "from lake",
            "from boss",
        },
    },
    {
        "spirit temple",
        5,
        (const char*[]){
            "from colossus",
            "from boss",
            "from right hand",
            "from left hand",
            "(crashes)",
        },
    },
    {
        "shadow temple",
        4,
        (const char*[]){
            "from graveyard",
            "outside boss",
            "from boss (crashes)",
            "(crashes)",
        },
    },
    {
        "bottom of the well",
        2,
        (const char*[]){
            "from kakariko",
            "from kakariko (unused)",
        },
    },
    {
        "ice cavern",
        2,
        (const char*[]){
            "from fountain",
            "near block puzzle room",
        },
    },
    {
        "ganon's tower",
        3,
        (const char*[]){
            "from castle",
            "from boss",
            "near boss door",
        },
    },
    {
        "gerudo training ground",
        1,
        (const char*[]){
            "from fortress",
        },
    },
    {
        "thieves' hideout",
        13,
        (const char*[]){
            "1-1",
            "1-2",
            "1-3",
            "2-3",
            "1-4",
            "1-2",
            "1-4",
            "3-3",
            "3-2",
            "4-1",
            "1-1",
            "prison",
            "3-1",
        },
    },
    {
        "ganon's castle",
        8,
        (const char*[]){
            "from castle",
            "from tower",
            "forest trial",
            "water trial",
            "shadow trial",
            "fire trial",
            "light trial",
            "spirit trial",
        },
    },
    {
        "tower collapse",
        8,
        (const char*[]){
            "boss room 1",
            "boss room 2",
            "rocks 1",
            "rocks 2",
            "stalfos 1",
            "stalfos 2",
            "staircase 1",
            "staircase 2",
        },
    },
    {
        "castle collapse",
        1,
        (const char*[]){
            "from tower",
        },
    },
    {
        "treasure box shop",
        1,
        (const char*[]){
            "from market",
        },
    },
    {
        "gohma",
        1,
        (const char*[]){
            "from deku tree",
        },
    },
    {
        "king dodongo",
        1,
        (const char*[]){
            "from dodongo's cavern",
        },
    },
    {
        "barinade",
        1,
        (const char*[]){
            "from jabu jabu",
        },
    },
    {
        "phantom ganon",
        1,
        (const char*[]){
            "from temple",
        },
    },
    {
        "volvagia",
        1,
        (const char*[]){
            "from temple",
        },
    },
    {
        "morpha",
        1,
        (const char*[]){
            "from temple",
        },
    },
    {
        "twinrova",
        3,
        (const char*[]){
            "from temple",
            "(unused)",
            "arena",
        },
    },
    {
        "bongo bongo",
        1,
        (const char*[]){
            "from temple",
        },
    },
    {
        "ganondorf",
        1,
        (const char*[]){
            "from tower",
        },
    },
    {
        "tower exterior",
        8,
        (const char*[]){
            "from ganondorf",
            "from boss 1",
            "from boss 2",
            "from rocks 1",
            "from rocks 2",
            "from stalfos 1",
            "from stalfos 2",
            "from staircase",
        },
    },
    {
        "market entrance day",
        3,
        (const char*[]){
            "from market",
            "from field",
            "from pots",
        },
    },
    {
        "market entrance night",
        3,
        (const char*[]){
            "from market",
            "from field",
            "from pots",
        },
    },
    {
        "market entrance ruins",
        3,
        (const char*[]){
            "from market",
            "from field",
            "from pots",
        },
    },
    {
        "back alley day",
        5,
        (const char*[]){
            "from market right",
            "from market left",
            "from bombchu shop",
            "from guy's house",
            "from richard's house",
        },
    },
    {
        "back alley night",
        5,
        (const char*[]){
            "from market right",
            "from market left",
            "from bombchu shop",
            "from guy's house",
            "from richard's house",
        },
    },
    {
        "market day",
        11,
        (const char*[]){
            "from entrance",
            "from castle",
            "from temple",
            "from alley right",
            "from alley left",
            "from potion shop",
            "from bazaar",
            "from bowling alley",
            "from shooting gallery",
            "from mask shop",
            "from box shop",
        },
    },
    {
        "market night",
        11,
        (const char*[]){
            "from entrance",
            "from castle",
            "from temple",
            "from alley right",
            "from alley left",
            "from potion shop",
            "from bazaar",
            "from bowling alley",
            "from shooting gallery",
            "from mask shop",
            "from box shop",
        },
    },
    {
        "market ruins",
        11,
        (const char*[]){
            "from entrance",
            "from castle",
            "from temple",
            "from alley right",
            "from alley left",
            "from potion shop",
            "from bazaar",
            "from bowling alley",
            "from shooting gallery",
            "from mask shop",
            "from box shop",
        },
    },
    {
        "outside temple day",
        2,
        (const char*[]){
            "from market",
            "from temple",
        },
    },
    {
        "outside temple night",
        2,
        (const char*[]){
            "from market",
            "from temple",
        },
    },
    {
        "outside temple ruins",
        2,
        (const char*[]){
            "from market",
            "from temple",
        },
    },
    {
        "brothers' house",
        1,
        (const char*[]){
            "from forest",
        },
    },
    {
        "twins' house",
        1,
        (const char*[]){
            "from forest",
        },
    },
    {
        "mido's house",
        1,
        (const char*[]){
            "from forest",
        },
    },
    {
        "saria's house",
        1,
        (const char*[]){
            "from forest",
        },
    },
    {
        "carpenter's house",
        1,
        (const char*[]){
            "from kakariko",
        },
    },
    {
        "guy's house",
        1,
        (const char*[]){
            "from alley",
        },
    },
    {
        "bazaar",
        2,
        (const char*[]){
            "from kakariko",
            "from market",
        },
    },
    {
        "kokiri shop",
        1,
        (const char*[]){
            "from forest",
        },
    },
    {
        "goron shop",
        1,
        (const char*[]){
            "from goron city",
        },
    },
    {
        "zora shop",
        1,
        (const char*[]){
            "from domain",
        },
    },
    {
        "kakariko potion shop",
        3,
        (const char*[]){"from front", "from front (unused)", "from back"},
    },
    {
        "market potion shop",
        1,
        (const char*[]){
            "from market",
        },
    },
    {
        "bombchu shop",
        2,
        (const char*[]){"from alley (unused)", "from alley"},
    },
    {
        "happy mask shop",
        1,
        (const char*[]){
            "from market",
        },
    },
    {
        "link's house",
        2,
        (const char*[]){"from bed", "from forest"},
    },
    {
        "richard's house",
        1,
        (const char*[]){
            "from alley",
        },
    },
    {
        "stable",
        1,
        (const char*[]){
            "from ranch",
        },
    },
    {
        "impa's house",
        2,
        (const char*[]){"from front", "from roof"},
    },
    {
        "lakeside laboratory",
        2,
        (const char*[]){
            "from lake",
            "eyedrops timeout",
        },
    },
    {
        "carpenter's tent",
        1,
        (const char*[]){
            "from valley",
        },
    },
    {
        "gravekeeper's hut",
        1,
        (const char*[]){
            "from graveyard",
        },
    },
    {
        "fairy upgrades",
        4,
        (const char*[]){
            "from trail",
            "from crater",
            "from castle",
            "from kakariko",
        },
    },
    {
        "fairy's fountain",
        1,
        (const char*[]){
            "grotto",
        },
    },
    {
        "fairy spells",
        3,
        (const char*[]){
            "from fountain",
            "from castle",
            "from colossus",
        },
    },
    {
        "grottos",
        14,
        (const char*[]){
            "default",
            "skulltula",
            "one scrub",
            "redeads",
            "three scrubs",
            "webbed",
            "octoroc",
            "two scrubs (nut)",
            "wolfos",
            "bombable wall",
            "two scrubs (potion)",
            "tektite",
            "stage",
            "cow",
        },
    },
    {
        "redead grave",
        1,
        (const char*[]){
            "from graveyard",
        },
    },
    {
        "shield grave",
        1,
        (const char*[]){
            "from graveyard",
        },
    },
    {
        "royal family's tomb",
        2,
        (const char*[]){
            "from graveyard",
            "after sun's song",
        },
    },
    {
        "shooting gallery",
        2,
        (const char*[]){
            "from kakariko",
            "from market",
        },
    },
    {
        "temple of time",
        8,
        (const char*[]){
            "from outside",
            "(unused)",
            "from master sword",
            "near stones",
            "after sheik",
            "after light arrow",
            "after light arrow (unused)",
            "from prelude",
        },
    },
    {
        "chamber of the sages",
        2,
        (const char*[]){
            "default",
            "unknown",
        },
    },
    {
        "courtyard day",
        2,
        (const char*[]){
            "from castle",
            "from zelda",
        },
    },
    {
        "courtyard night",
        2,
        (const char*[]){
            "from castle",
            "from zelda",
        },
    },
    {
        "cutscene map",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "windmill",
        3,
        (const char*[]){
            "from graveyard",
            "from kakariko",
            "after song of storms",
        },
    },
    {
        "fishing pond",
        1,
        (const char*[]){
            "from lake",
        },
    },
    {
        "zelda",
        2,
        (const char*[]){
            "from courtyard",
            "after triforce",
        },
    },
    {
        "bombchu bowling alley",
        1,
        (const char*[]){
            "from market",
        },
    },
    {
        "ranch buildings",
        3,
        (const char*[]){
            "talon's house",
            "tower",
            "after cucco game",
        },
    },
    {
        "lots 'o pots",
        1,
        (const char*[]){
            "from entrance",
        },
    },
    {
        "granny's potion shop",
        1,
        (const char*[]){
            "from kakariko",
        },
    },
    {
        "ganon",
        1,
        (const char*[]){
            "from castle collapse",
        },
    },
    {
        "house of skulltula",
        1,
        (const char*[]){
            "from kakariko",
        },
    },
    {
        "hyrule field",
        18,
        (const char*[]){
            "impa escort",
            "from kakariko",
            "from river land",
            "from woods",
            "from lake",
            "from valley",
            "from ranch",
            "from market",
            "near lake",
            "owl escort",
            "near castle river",
            "south fence jump",
            "west fence jump",
            "east fence jump",
            "from river water",
            "gate jump",
            "after song of time",
            "after impa cutscene",
        },
    },
    {
        "kakariko village",
        16,
        (const char*[]){
            "from field",
            "from trail",
            "from graveyard",
            "from bazaar",
            "from well",
            "impa's door",
            "from carpenter's house",
            "from granny's shop",
            "from windmill",
            "potion shop front",
            "from shooting gallery",
            "from house of skulltula",
            "potion shop back",
            "after nocturne",
            "owl escort",
            "impa's roof",
        },
    },
    {
        "graveyard",
        9,
        (const char*[]){
            "from kakariko",
            "from shadow temple",
            "from hut",
            "from dampe's grave",
            "from shield grave",
            "from redead grave",
            "from tomb",
            "from nocturne",
            "from blue warp",
        },
    },
    {
        "zora's river",
        5,
        (const char*[]){
            "from field land",
            "waterfall",
            "from domain",
            "from field water",
            "from woods",
        },
    },
    {
        "kokiri forest",
        13,
        (const char*[]){
            "medallion cutscene",
            "from deku tree",
            "from bridge",
            "from link's house",
            "from shop",
            "from brothers' house",
            "from woods",
            "top of link's house",
            "from twins' house",
            "from mido's house",
            "from saria's house",
            "after emerald",
            "deku tree sprout",
        },
    },
    {
        "sacred forest meadow",
        4,
        (const char*[]){
            "from woods",
            "from temple",
            "from minuet",
            "from blue warp",
        },
    },
    {
        "lake hylia",
        10,
        (const char*[]){
            "from field",
            "from valley",
            "from temple",
            "near field",
            "from laboratory",
            "near pedestal",
            "from pond",
            "from domain",
            "from serenade",
            "from blue warp",
        },
    },
    {
        "zora's domain",
        5,
        (const char*[]){
            "from river",
            "from fountain",
            "from shop",
            "frog timeout",
            "from lake",
        },
    },
    {
        "zora's fountain",
        6,
        (const char*[]){
            "after sapphire",
            "from jabu jabu",
            "from domain",
            "from cavern",
            "south-west corner",
            "from fairy's fountain",
        },
    },
    {
        "gerudo valley",
        5,
        (const char*[]){
            "from field",
            "thrown out",
            "near fortress",
            "from fortress",
            "from tent",
        },
    },
    {
        "lost woods",
        10,
        (const char*[]){
            "from forest",
            "from meadow",
            "mushroom timeout",
            "unknown",
            "unknown",
            "unknown",
            "from goron city",
            "from river",
            "from field",
            "from forest exit",
        },
    },
    {
        "desert colossus",
        9,
        (const char*[]){
            "from wasteland",
            "from temple",
            "right hand",
            "left hand",
            "after requiem",
            "from requiem",
            "after gauntlets",
            "from fairy's fountain",
            "from blue warp",
        },
    },
    {
        "gerudo's fortress",
        19,
        (const char*[]){
            "from valley",
            "1-1",
            "1-2",
            "1-3",
            "2-3",
            "1-4",
            "2-2",
            "2-4",
            "3-3",
            "3-2",
            "4-1",
            "2-1",
            "above prison",
            "3-1",
            "from training grounds",
            "from wasteland",
            "after archery game",
            "captured first time",
            "captured 2nd+ time",
        },
    },
    {
        "haunted wasteland",
        3,
        (const char*[]){
            "from fortress",
            "from colossus",
            "(crashes)",
        },
    },
    {
        "hyrule castle",
        5,
        (const char*[]){
            "from market",
            "from courtyard",
            "from fairy's fountain",
            "caught in courtyard",
            "caught outside",
        },
    },
    {
        "death mountain trail",
        6,
        (const char*[]){
            "from kakariko",
            "from goron city",
            "from crater",
            "from cavern",
            "from fairy's fountain",
            "after ruby",
        },
    },
    {
        "death mountain crater",
        6,
        (const char*[]){
            "from trail",
            "from goron city",
            "from temple",
            "from fairy's fountain",
            "from bolero",
            "from blue warp",
        },
    },
    {
        "goron city",
        4,
        (const char*[]){
            "from trail",
            "from crater",
            "from shop",
            "from woods",
        },
    },
    {
        "lon lon ranch",
        11,
        (const char*[]){
            "from field",
            "after epona's song",
            "south gate",
            "west gate",
            "from talon's house",
            "from stable",
            "horse riding",
            "after beating ingo",
            "after losing to ingo",
            "near ingo / malon",
            "from tower",
        },
    },
    {
        "ganon's castle",
        5,
        (const char*[]){"from market", "from castle", "from fairy's fountain", "caught in courtyard", "(crashes)"},
    },
    {
        "collision test",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "ganondorf test",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "depth test",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "stalfos middle room",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "stalfos boss room",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "dark link test",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "castle courtyard",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "action test",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "item test",
        1,
        (const char*[]){
            "default",
        },
    },
    {
        "lost scene",
        1,
        (const char*[]){
            "default",
        },
    },
};
