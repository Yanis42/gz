...
static void save_ovl(void** p, void* addr, uint32_t vrom_start, uint32_t vrom_end) {
    ...
    z64_ovl_hdr_t* hdr;
#if Z64_VERSION == Z64_OOT10 || Z64_VERSION == Z64_OOT11 || Z64_VERSION == Z64_OOT12
    hdr = (void*)(end - *hdr_off);
#elif Z64_VERSION == Z64_OOTMQJ || Z64_VERSION == Z64_OOTMQU || Z64_VERSION == Z64_OOTGCJ || \
    Z64_VERSION == Z64_OOTGCU || Z64_VERSION == Z64_OOTCEJ
    z64_ovl_hdr_t l_hdr;
    hdr = &l_hdr;
...
static void load_ovl(void** p, void** p_addr, uint32_t vrom_start, uint32_t vrom_end, uint32_t vram_start,
                     uint32_t vram_end) {
    /* locate file table entry */
    z64_ftab_t* file = NULL;
    for (int i = 0;; ++i) {
...
static void grayscale_texture(uint32_t* pixels, uint16_t n_pixels) {
    for (int i = 0; i < n_pixels; ++i) {
        uint32_t c = pixels[i];
        uint8_t s = (((c & 0xFF000000) >> 24) + ((c & 0x00FF0000) >> 15) + ((c & 0x0000FF00) >> 8)) / 7;
...
uint32_t save_state(struct state_meta* state) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (seq_active) {
            ...
            serial_write(&p, &sc->stop_cmd_buf, sizeof(*sc->stop_cmd_buf) * sc->stop_cmd_count);
        }
    ...
    for (z64_arena_node_t* node = z64_game_arena.first_node; node; node = node->next) {
        serial_write(&p, &sot, sizeof(sot));
    ...
    /* save transition actor list (it may have been modified during gameplay) */
    {
        ...
        serial_write(&p, room_ctxt->tnsn_list, room_ctxt->n_tnsn * sizeof(*room_ctxt->tnsn_list));
    }
    ...
    /* save waterboxes */
    {
        ...
        serial_write(&p, col_hdr->water, sizeof(*col_hdr->water) * col_hdr->n_water);
    }
    /* save dynamic collision */
    {
        z64_col_ctxt_t* col = &z64_game.col_ctxt;
        serial_write(&p, col->dyn_list, col->dyn_list_max * sizeof(*col->dyn_list));
        serial_write(&p, col->dyn_poly, col->dyn_poly_max * sizeof(*col->dyn_poly));
        serial_write(&p, col->dyn_vtx, col->dyn_vtx_max * sizeof(*col->dyn_vtx));
    }
...
void load_state(const struct state_meta* state) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (si->p_active) {
            ...
            serial_read(&p, &si->stop_cmd_buf, sizeof(*si->stop_cmd_buf) * si->stop_cmd_count);
        }
    ...
    for (int16_t i = 0; i < n_ent; ++i) {
        ...
        if (i == next_ent) {
            ...
            load_ovl(&p, &ovl->ptr, ovl->vrom_start, ovl->vrom_end, ovl->vram_start, ovl->vram_end);
            set_insert(&ovl_nodes, &ovl->ptr);
            serial_read(&p, &next_ent, sizeof(next_ent));
        } else {
            ovl->n_inst = 0;
    ...
    while (node) {
        ...
        node->filename = NULL, node->line = 0;
        node->thread_id = 4;
#endif
    ...
    if (z64_game.scene_index != scene_index) {
        ...
        z64_CreateStaticCollision(&z64_game.col_ctxt, &z64_game, z64_game.col_ctxt.stc_lut);
    }
    {
        /* load transition actor list */
        z64_room_ctxt_t* room_ctxt = &z64_game.room_ctxt;
        serial_read(&p, room_ctxt->tnsn_list, room_ctxt->n_tnsn * sizeof(*room_ctxt->tnsn_list));
        /* load rooms */
        for (int i = 0; i < 2; ++i) {
            ...
            if (c_ptr && c_id != -1 && (z64_game.scene_index != scene_index || c_id != p_id || c_ptr != p_ptr)) {
    ...
    if (c_pause_objects) {
        ...
        if (c_gameover) {
            zu_getfile_idx(z64_icon_item_gameover_static, z64_game.pause_ctxt.icon_item_s);
        } else {
            z64_InitPauseObjects(&z64_game, z64_game.pause_ctxt.p_0x13C, &z64_game.pause_ctxt.s_0x27C);
            if (dungeon_map) {
                zu_getfile_idx(z64_icon_item_dungeon_static, z64_game.pause_ctxt.icon_item_s);
                ...
                zu_getfile_idx(z64_icon_item_field_static, z64_game.pause_ctxt.icon_item_s);
            }
        ...
        if (z64_file.language == 0) {
            zu_getfile_idx(z64_icon_item_jpn_static, z64_game.pause_ctxt.icon_item_lang);
        } else {
            zu_getfile_idx(z64_icon_item_nes_static, z64_game.pause_ctxt.icon_item_lang);
        }
        ...
        for (int i = 0; i < 0x56; ++i) {
            if (item_highlight_tab[i] != 0x09 && item_highlight_tab[i] != z64_file.link_age) {
    ...
    /* load waterboxes */
    {
        ...
        serial_read(&p, col_hdr->water, sizeof(*col_hdr->water) * col_hdr->n_water);
    }
    /* load dynamic collision */
    {
        z64_col_ctxt_t* col = &z64_game.col_ctxt;
        serial_read(&p, col->dyn_list, col->dyn_list_max * sizeof(*col->dyn_list));
        serial_read(&p, col->dyn_poly, col->dyn_poly_max * sizeof(*col->dyn_poly));
        serial_read(&p, col->dyn_vtx, col->dyn_vtx_max * sizeof(*col->dyn_vtx));
    }
    ...
    if (state->state_version >= 0x0004) {
        serial_read(&p, &z64_message_select_state, sizeof(z64_message_select_state));
    }
    ...
    /* action labels */
    z64_LoadActionLabel(&z64_game.if_ctxt, z64_game.if_ctxt.a_action, Z64_ACTIONBTN_A);
    z64_LoadActionLabel(&z64_game.if_ctxt, z64_game.if_ctxt.b_label % 0x1D, Z64_ACTIONBTN_B);
    ...
    if (z64_game.message_state_1 != 0) {
        ...
        switch (z64_game.message_type) {
            case 0:
                zu_getfile(message_static + 0x0000, tex_buf, 0x1000);
                break;
            case 1:
                zu_getfile(message_static + 0x1000, tex_buf, 0x1000);
                break;
            case 2:
                zu_getfile(message_static + 0x3000, tex_buf, 0x1000);
                break;
            case 3:
                zu_getfile(message_static + 0x2000, tex_buf, 0x1000);
                break;
            default:
                break;
        }
        ...
        if (foreground_image == 0) {
            zu_getfile(z64_ftab[z64_message_texture_static].vrom_start, tex_buf + 0x1000, 0x1200);
        } else if (icon_idx >= 0x66) {
            zu_getfile(z64_ftab[z64_icon_item_24_static].vrom_start + (icon_idx - 0x66) * 0x900, tex_buf + 0x1000,
                       0x900);
        } else if (icon_idx >= 0x00) {
            zu_getfile(z64_ftab[z64_icon_item_static].vrom_start + icon_idx * 0x1000, tex_buf + 0x1000, 0x1000);
        }
    }
    ...
    if (next_ent == 0) {
        ...
        gDPSetColorImage(gfx->work.p++, G_IM_FMT_RGBA, G_IM_SIZ_16b, Z64_SCREEN_WIDTH, ZU_MAKE_SEG(Z64_SEG_CIMG, 0));
        gDPSetCycleType(gfx->work.p++, G_CYC_FILL);
        gDPSetRenderMode(gfx->work.p++, G_RM_NOOP, G_RM_NOOP2);
        gDPSetFillColor(gfx->work.p++,
                        (GPACK_RGBA5551(0x1F, 0x1F, 0x1F, 0x01) << 16) | GPACK_RGBA5551(0x1F, 0x1F, 0x1F, 0x01));
        gDPFillRectangle(gfx->work.p++, 0, 0, Z64_SCREEN_WIDTH - 1, Z64_SCREEN_HEIGHT - 1);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        if (si->p_active) {
            memcpy(&sc->stop_cmd_timer, &si->stop_cmd_timer, sizeof(si->stop_cmd_timer));
            memcpy(&sc->stop_cmd_count, &si->stop_cmd_count, sizeof(si->stop_cmd_count));
            memcpy(&sc->stop_cmd_buf, &si->stop_cmd_buf, sizeof(*si->stop_cmd_buf) * si->stop_cmd_count);
    ...
    /* restore pending audio commands */
    {
        ...
        for (uint8_t i = 0; i != n_cmd; ++i) {
            serial_read(&p, &z64_audio_cmd_buf[z64_audio_cmd_write_pos++], sizeof(*z64_audio_cmd_buf));
        }
    }
#if 0
  {
    ...
    Z64_VERSION == Z64_OOTGCU || Z64_VERSION == Z64_OOTCEJ
        ...
        if (state->state_version >= 0x0004) {
            serial_read(&p, &z64_ocarina_button_state, sizeof(z64_ocarina_button_state));
        }
