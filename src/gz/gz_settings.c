...
static int input_pressrel_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
        settings->bits.input_pressrel = 1;
    } else if (reason == MENU_CALLBACK_SWITCH_OFF) {
        settings->bits.input_pressrel = 0;
    } else if (reason == MENU_CALLBACK_THINK) {
        menu_checkbox_set(item, settings->bits.input_pressrel);
    }
    return 0;
}
...
static int lag_counter_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int lag_unit_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_THINK_INACTIVE) {
...
static int timer_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int pause_display_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int macro_input_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_SWITCH_ON) {
...
static int break_type_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    if (reason == MENU_CALLBACK_THINK_INACTIVE) {
...
static int generic_position_proc(struct menu_item* item, enum menu_callback_reason reason, void* data) {
    ...
    switch (reason) {
        case MENU_CALLBACK_ACTIVATE:
            input_reserve(BUTTON_Z);
            break;
        case MENU_CALLBACK_DEACTIVATE:
            input_free(BUTTON_Z);
            break;
        case MENU_CALLBACK_NAV_UP:
            *y -= dist;
            break;
        case MENU_CALLBACK_NAV_DOWN:
            *y += dist;
            break;
        case MENU_CALLBACK_NAV_LEFT:
            *x -= dist;
            break;
        case MENU_CALLBACK_NAV_RIGHT:
            *x += dist;
            break;
        default:
            break;
...
static void tab_prev_proc(struct menu_item* item, void* data) { menu_tab_previous(data); }

static void tab_next_proc(struct menu_item* item, void* data) { menu_tab_next(data); }
...
struct menu* gz_settings_menu(void) {
    ...
    menu_add_positioning(&menu, 18, 5, generic_position_proc, &settings->input_display_x);
    menu_add_static(&menu, 0, 6, "button details", 0xC0C0C0);
    menu_add_checkbox(&menu, 16, 6, input_pressrel_proc, NULL);
    menu_add_static(&menu, 0, 7, "log", 0xC0C0C0);
    menu_add_checkbox(&menu, 16, 7, log_proc, NULL);
    menu_add_positioning(&menu, 18, 7, log_position_proc, NULL);
    menu_add_static(&menu, 0, 8, "lag counter", 0xC0C0C0);
    menu_add_checkbox(&menu, 16, 8, lag_counter_proc, NULL);
    menu_add_positioning(&menu, 18, 8, generic_position_proc, &settings->lag_counter_x);
    menu_add_option(&menu, 20, 8,
                    "frames\0"
                    "seconds\0",
                    lag_unit_proc, NULL);
    menu_add_static(&menu, 0, 9, "timer", 0xC0C0C0);
    menu_add_checkbox(&menu, 16, 9, timer_proc, NULL);
    menu_add_positioning(&menu, 18, 9, generic_position_proc, &settings->timer_x);
    menu_add_static(&menu, 0, 10, "pause display", 0xC0C0C0);
    menu_add_checkbox(&menu, 16, 10, pause_display_proc, NULL);
    /* behavior controls */
    ...
    menu_add_button(&menu, 0, 16, "restore defaults", restore_settings_proc, NULL);
